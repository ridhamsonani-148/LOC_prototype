...
class DataCollector:
    ...
    def extract_text_with_bda(self, pdf_url: str, doc_id: str) -> str:
        ...
        try:
            ...
            output_key = f"temp/bda-output/{doc_id}/"
            ...
            response = bedrock_data_automation.invoke_data_automation_async(
                ...
                dataAutomationConfiguration={
                    'dataAutomationArn': f'arn:aws:bedrock:{boto3.Session().region_name}::data-automation/default',
                    'stage': 'DEVELOPMENT'
                },
            ...
            while elapsed < max_wait:
                status_response = bedrock_data_automation.get_data_automation_status(
                    invocationArn=invocation_arn
                )
                
                status = status_response['status']
                self.log(f"  BDA status: {status}")
                
                if status == 'Success':
                    # Extract text from BDA output
                    text = self._extract_text_from_bda_output(output_key)
                    
                    # Cleanup temp files
                    self._cleanup_s3_prefix(f"temp/pdfs/{doc_id}")
                    self._cleanup_s3_prefix(f"temp/bda-output/{doc_id}")
                    
                    return text
                elif status in ['Failed', 'Cancelled']:
                    self.log(f"  BDA processing failed: {status}")
                    return None
                
                time.sleep(10)
                elapsed += 10
            
            self.log(f"  BDA processing timeout after {max_wait}s")
            return None
            
        except Exception as e:
            self.log(f"  Error with BDA extraction: {str(e)}")
            # Fallback: try simple text extraction
            return self._fallback_text_extraction(pdf_url)
    
    def _extract_text_from_bda_output(self, output_prefix: str) -> str:
    ...
    def _fallback_text_extraction(self, url: str) -> str:
        """Fallback: simple text extraction if BDA fails"""
        try:
            self.log(f"  Using fallback text extraction")
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
            }
            ...
            # Simple text extraction (placeholder - would need proper PDF library)
